...
def _is_legacy_assets(route: Any) -> bool:
    return getattr(route, "path", None) == "/assets" and "GET" in getattr(route, "methods", set())


app = FastAPI(title=legacy.app.title)
app.router.routes.extend(
    route for route in legacy.app.router.routes
    if not _is_legacy_readings(route) and not _is_legacy_municipio_summary(route) and not _is_legacy_assets(route)
)
