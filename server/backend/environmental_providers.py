...
PROVIDERS: dict[str, Provider] = {
    "neon": Provider(
        ...
        "NEON_API_TOKEN",
        "X-API-Token",
        scope="GUAN, LAJA, CUPE, GUIL",
    ),
    "usgs": Provider(
        "usgs",
        "USGS Water Data APIs",
        "T1",
        "https://api.waterdata.usgs.gov/ogcapi/v0/collections/monitoring-locations/items?f=json&limit=1",
    ),
    "nws": Provider(
        "nws",
        "NOAA/NWS API",
        "T1",
        "https://api.weather.gov/points/18.2208,-66.5901",
    ...
    "nasa": Provider(
        ...
        "T1",
        "https://cmr.earthdata.nasa.gov/search/collections.json?page_size=1&keyword=GPM",
    ),
    "lter": Provider(
        "lter",
        "Luquillo LTER / USFS",
    ...
    "wqp": Provider(
        ...
        "T1",
        "https://www.waterqualitydata.us/data/Station/search?statecode=US:72&mimeType=json&zip=no&providers=NWIS&providers=STORET",
    ),
    "drna": Provider(
        "drna",
        "Puerto Rico DRNA hydrology",
        "T1",
...
NEON_PR_SITES = (
    ...
    {
        "site_code": "CUPE",
        "site_name": "Rio Cupeyes",
        "site_type": "aquatic",
    },
    {
        "site_code": "GUIL",
        "site_name": "Rio Yahuecas",
        "site_type": "aquatic",
    },
)
    ...
    return [
        {
            **asdict(provider),
            "configured": (
                bool(provider.auth_env and os.getenv(provider.auth_env))
                if provider.auth_env
                else True
            ),
            "polling_enabled": os.getenv(provider.enabled_env, "").lower()
            == "true",
        }
...
def _headers(provider: Provider) -> dict[str, str]:
    headers = {
        ...
        "User-Agent": "AguaYLuz/1.0 environmental-monitor",
    }
...
def poll_provider(code: str, *, timeout: float = 15.0) -> dict[str, Any]:
    ...
    if os.getenv(provider.enabled_env, "").lower() != "true":
        return {
            ...
            "reason": "external_polling_disabled",
        }
    ...
    if persist:
        ...
        if SNAPSHOT_PATH.exists():
            try:
                ...
                previous = {}
        previous_hashes = {
            item.get("provider"): item.get("sha256")
        ...
        snapshot["deltas"] = [
            {
                ...
                "changed": bool(
                    item.get("sha256")
                    and item.get("sha256") != previous_hashes.get(item["provider"])
                ),
            }
            for item in results
        ]
