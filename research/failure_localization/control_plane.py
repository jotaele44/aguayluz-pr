...
class FailureLocalizationControlPlane:
    ...
    def configure_graph(self, graph: dict[str, Any], *, idempotency_key: str) -> dict[str, Any]:
        ...
        return self.store.append_idempotent(
            "graph_snapshots", snapshot, idempotency_key=idempotency_key
        )

    def ingest_observation(
        self, observation: dict[str, Any], *, idempotency_key: str
    ) -> dict[str, Any]:
        ...
        return self.store.append_idempotent(
            "observations", row, idempotency_key=idempotency_key
        )

    def assess(
        self, *, as_of: str, system_id: str | None = None, idempotency_key: str
    ) -> dict[str, Any]:
        ...
        assets = {
            ...
            if system_id is None or value.get("system_id") == system_id
        }
        ...
        edges = {
            ...
            if item["from_asset_id"] in assets and item["to_asset_id"] in assets
        }
        ...
        if not candidates:
            candidates = [
                self._unknown_candidate(assets, materialized["stale_observation_ids"])
            ]
        seed = {
            ...
            "system_id": system_id,
            "candidates": [
                [item["hypothesis"], item["target_asset_ids"]] for item in candidates
            ],
            "observations": sorted(
                row.get("record_hash")
        ...
        assessment = {
            ...
            "assessment_id": stable_id("AYL_FLA", seed),
            "graph_id": graph["graph_id"],
            "as_of": at.isoformat(),
            "system_id": system_id,
            "localization_grades": LOCALIZATION_GRADES,
            "mass_balance": balances,
            "hydraulic_discontinuities": discontinuities,
            "outage_clusters": clusters,
        ...
        persisted = self.store.append_idempotent(
            "assessments", assessment, idempotency_key=idempotency_key
        )
    ...
    def promote_candidate(
        ...
        *,
        ...
        if candidate["localization_grade"] != required:
            raise ValueError(
                f"promotion_sequence_violation:{candidate['localization_grade']}:{requested_grade}"
            )
        at = parse_timestamp(occurred_at)
        evidence = self._observation_records(evidence_observation_ids)
        ...
        if not any(check(row, candidate, at) for row in evidence):
            error = (
                "l4_authoritative_exact_asset_evidence_required"
                if requested_grade == "L4"
                else "l5_field_confirmation_required"
            )
        ...
        self.store.append_idempotent(
            "promotion_events", event, idempotency_key=idempotency_key
        )
        return self._materialize_assessment(assessment_id)

    def current_assessment(
        self, assessment_id: str, *, view_mode: str = "public"
    ) -> dict[str, Any]:
        assessment = self._materialize_assessment(assessment_id)
    ...
    def _materialize_observations(self, at):
        ...
        for row in self.store.read("observations"):
            if row["review_status"] == "rejected" or row["quality"] == "invalid":
                rejected.append(row["observation_id"])
                continue
            observed = parse_timestamp(row["observed_at"])
            if observed > at:
                future.append(row["observation_id"])
                continue
            if (at - observed).total_seconds() > row["max_age_seconds"]:
                stale.append(row["observation_id"])
                continue
            target = (
                ("asset", row["asset_id"])
                if row.get("asset_id")
                else ("edge", row["edge_id"])
            )
            kind, target_id = target
            series[(kind, target_id, row["metric"])].append(row)
        latest = {}
        ...
        return {
            "latest": latest,
            "series": series,
            "stale_observation_ids": sorted(stale),
            "rejected_observation_ids": sorted(rejected),
            "future_observation_ids": sorted(future),
        }

    @staticmethod
    def _unknown_candidate(assets, stale):
        system_ids = unique(
            str(item.get("system_id") or "") for item in assets.values()
        )
        return {
            "candidate_id": stable_id("AYL_FLC", {"unknown": system_ids}),
            "rank": 1,
            "hypothesis": "unknown",
            "localization_grade": "L0",
            "maximum_inference_grade": "L3",
            "target_asset_ids": [],
            "target_edge_ids": [],
            "pressure_zone_ids": [],
            ...
            "required_field_tests": ["systematic_field_survey"],
    ...
    def _binds(row, candidate):
        return (
            row.get("asset_id") in candidate["target_asset_ids"]
            or row.get("edge_id") in candidate["target_edge_ids"]
            or bool(
                set(row.get("related_asset_ids", [])).intersection(
                    candidate["target_asset_ids"]
                )
            )
        )
    ...
    def _is_l4_evidence(self, row, candidate, at):
        return (
            ...
            and row["evidence_tier"] == "T1"
            and row["authoritative"]
            and self._current(row, at)
            and self._binds(row, candidate)
            and row["assertion"] in L4_ASSERTIONS
        )
    ...
    def _is_l5_evidence(self, row, candidate, at):
        return (
            ...
            and row["evidence_tier"] == "T1"
            and row["authoritative"]
            and row["field_confirmed"]
            and self._current(row, at)
            and self._binds(row, candidate)
            and row["assertion"] in L5_ASSERTIONS
        )
    ...
    def _materialize_assessment(self, assessment_id):
        ...
        events = [
            row
            for row in self.store.read("promotion_events")
        ...
        for candidate in output["candidates"]:
            ...
            if history:
                ...
                candidate["promotion"]["l4_eligible"] = any(
                    item["to_grade"] == "L4" for item in history
                )
                candidate["promotion"]["l5_eligible"] = any(
                    item["to_grade"] == "L5" for item in history
                )
    ...
    def _redact_public(self, assessment):
        ...
        for candidate in assessment["candidates"]:
            ...
            for asset_id in candidate["target_asset_ids"]:
                ...
                restricted_control = (
                    asset.get("asset_type") in CONTROL_ASSET_TYPES
                    and asset.get("disclosure") != "public_exact"
                )
                if restricted_control:
                    visible.append(stable_id("AYL_REDACTED", asset_id, 16))
                    count += 1
                else:
                    visible.append(asset_id)
            if count:
                candidate["target_asset_ids"] = visible
                candidate["target_edge_ids"] = []
                candidate["public_redaction"] = {
                    "redacted_control_asset_count": count,
                    "exact_operator_details_withheld": True,
                }
        return assessment
