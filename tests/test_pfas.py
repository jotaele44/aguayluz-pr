from aguayluz.pfas import (
    RegulatoryRule,
    certification_report,
    compare_measurement_to_rule,
    current_rule,
    normalize_ng_l,
    validate_binding,
...
def test_unit_normalization():
    assert normalize_ng_l(0.0467, "ug/L") == 46.7
    assert normalize_ng_l(4.0, "ng/L") == 4.0
...
def test_proposed_rescission_does_not_erase_current_rule():
    rule = RegulatoryRule(
        "EPA_NPDWR_2024_PFHXS_MCL", "PFHxS", 10.0, "ng/L", "MCL",
        "IN_FORCE_PROPOSED_RESCISSION", "2024-06-25", "2029-04-26", "epa",
    )
    assert current_rule([rule], "PFHxS", "2026-09-03") == rule


def test_occurrence_measurement_never_becomes_compliance_finding():
    rule = RegulatoryRule(
        "EPA_NPDWR_2024_PFOS_MCL", "PFOS", 4.0, "ng/L", "MCL",
        "IN_FORCE", "2024-06-25", "2029-04-26", "epa",
    )
    result = compare_measurement_to_rule(_measurement(), rule, as_of="2026-09-03")
...
def test_missing_source_hash_keeps_certification_open():
    result = certification_report(
        source_manifestations=[{
            "source_record_id": "s1", "url": "https://example.test/source",
            "retrieved_utc": "2026-09-03T17:00:00Z", "byte_sha256": None,
        }],
