...
def _receipt_for(case_name: str = "valid"):
    ...
    response = _response(case, request)
    return snapshot._receipt(response, "2026-08-08T22:30:00Z")
...
def test_valid_observation_normalizes_required_fields_and_receipt_hash():
    ...
    row = snapshot.normalize_observation(
        feature,
        _receipt_for(),
    ...
    required = {
        "source_id", "monitoring_location", "parameter_code", "observed_property",
        "value", "unit", "datum", "observed_at", "qualifier", "approval_status",
        "received_at", "request_receipt_sha256", "evidence_tier", "quality_flag",
        "privacy_class", "operational_admission",
    }
...
def test_partial_observation_is_not_promoted():
    ...
    assert snapshot.normalize_observation(
        feature,
        _receipt_for("partial"),
        collection="latest-continuous",
    ) is None
...
def test_50014600_and_50014800_precipitation_are_historical_only():
    ...
    historical_site = {
        "properties": {
            "monitoring_location_id": "USGS-50014600", "parameter_code": "00060",
            "value": 12.0, "unit_of_measure": "ft3/s", "time": "1996-01-01T00:00:00Z",
        }
    }
    historical_rain = {
        "properties": {
            "monitoring_location_id": "USGS-50014800", "parameter_code": "00045",
            "value": 0.2, "unit_of_measure": "in", "time": "2022-01-01T00:00:00Z",
        }
    }
...
def test_pagination_follows_next_link_and_hashes_each_page():
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        case = cases["paginated_second"] if request.url.params.get("cursor") == "page2" else cases["paginated_first"]
        return _response(case, request)
...
@pytest.mark.parametrize(
    ("case_name", "expected_code"),
    [("unknown_parameter", "unknown_parameter"), ("out_of_scope_site", "out_of_scope_site")],
)
...
def test_failed_snapshot_degrades_to_unknown_and_never_access_action():
    ...
    with httpx.Client(transport=httpx.MockTransport(handler)) as client:
        result = snapshot.materialize_snapshot(
            client,
            now=datetime(2026, 8, 8, 22, 30, tzinfo=timezone.utc),
        )
...
def test_internal_projection_is_p0_only_and_contains_no_emergency_geometry():
    synthetic = {
        ...
        "generated_at": "2026-08-08T22:30:00Z",
        ...
        "diagnostics": [],
        "emergency_geometry": {"route": "restricted"},
    }
