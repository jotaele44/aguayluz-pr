...
def test_build_sqlite_in_memory_loads_all_tables():
    ...
    try:
        ...
        conn.close()
    assert counts == {"alert_modules": 10, "alert_events": 10,
                      "alert_dependency_edges": 5, "alert_gaps": 5}
...
def test_geojson_projection_skips_null_coords():
    geo = events_to_geojson(load_events())
    assert geo["type"] == "FeatureCollection"
    # only the Carraízo seed carries coordinates
    assert len(geo["features"]) == 1
    feat = geo["features"][0]
    assert feat["geometry"]["coordinates"] == [-66.01, 18.344]
    assert feat["properties"]["module_id"] == "HYDRO_OPS"
