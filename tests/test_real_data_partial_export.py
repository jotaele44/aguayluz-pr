...
@pytest.mark.parametrize(
    ("id_field", "record_id"),
    (("asset_id", "AYL_AST_DUP"), ("event_id", "AYL_EVT_DUP"), ("edge_id", "EDGE-WP-DUP")),
)
def test_duplicate_stable_ids_fail_closed(id_field: str, record_id: str) -> None:
    rows = [{id_field: record_id}, {id_field: record_id}]
    with pytest.raises(ValueError, match=f"duplicate {id_field}: {record_id}"):
        _builder._require_unique_ids(rows, id_field, "fixture")


def test_missing_stable_id_fails_closed() -> None:
    with pytest.raises(ValueError, match="missing asset_id"):
        _builder._require_unique_ids([{"asset_id": ""}], "asset_id", "fixture")
