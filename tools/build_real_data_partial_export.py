...
def _require_unique_ids(rows: list[dict[str, Any]], id_field: str, source: str) -> None:
    seen: set[str] = set()
    for idx, row in enumerate(rows):
        value = row.get(id_field)
        record_id = str(value) if value not in (None, "") else ""
        if not record_id:
            raise ValueError(f"{source} row {idx}: missing {id_field}")
        if record_id in seen:
            raise ValueError(f"{source}: duplicate {id_field}: {record_id}")
        seen.add(record_id)
...
def build(args: argparse.Namespace) -> dict[str, Any]:
    ...
    _require_unique_ids(asset_rows, "asset_id", str(assets_path))
    _require_unique_ids(event_rows, "event_id", str(events_path))
    _require_unique_ids(dependency_rows, "edge_id", str(deps_path))
    _require_unique_ids(recovery_rows, "project_id", str(recovery_path))
    _require_unique_ids(source_registry.get("sources", []), "source_id", str(registry_path))
    _require_unique_ids(taxonomy.get("classes", []), "risk_type", str(taxonomy_path))
    ...
    _require_unique_ids(selected_assets, "asset_id", "selected utility assets")
    _require_unique_ids(selected_outages, "event_id", "selected outage events")
    _require_unique_ids(selected_recovery, "project_id", "selected recovery projects")
    _require_unique_ids(continuity, "edge_id", "derived continuity edges")
