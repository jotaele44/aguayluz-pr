...
NID_LAYERS = {"Dams"}
NHDPLUS_LAYERS = {
    "nhdarea_pr": ("water", "nhd_area", "polygon", "USGS/EPA"),
    "nhdline_pr": ("water", "nhd_line", "line", "USGS/EPA"),
    "nhdpoint_pr": ("water", "nhd_point", "point", "USGS/EPA"),
    "nhdflowline_pr": ("water", "nhd_flowline", "line", "USGS/EPA"),
    "nhdwaterbody_pr": ("water", "nhd_waterbody", "polygon", "USGS/EPA"),
    "catchment_fabric_pr": ("water", "nhdplus_catchment", "polygon", "USGS/EPA"),
}
WETLAND_LAYERS = {
    "Wetlands": ("water", "wetland", "polygon", "USFWS"),
    "wetlands": ("water", "wetland", "polygon", "USFWS"),
    "PRVI_Wetlands": ("water", "wetland", "polygon", "USFWS"),
    "PRVI_Wetlands_Metadata": ("water", "wetland_project_metadata", "polygon", "USFWS"),
    "PuertoRico": ("water", "wetlands_region_boundary", "polygon", "USFWS"),
}
GAZETTEER_LAYERS = {
    "Gaz_Features": ("water", "gazetteer_hydro_feature", "point", "USGS GNIS"),
    "DomesticNames": ("water", "gazetteer_hydro_name", "point", "USGS GNIS"),
    "HistoricalFeatures": ("water", "historic_hydro_place", "point", "USGS GNIS"),
}
GAZETTEER_WATER_CLASSES = {
    "Arroyo", "Bay", "Bend", "Canal", "Channel", "Falls", "Flat", "Gut", "Harbor",
    "Lake", "Reservoir", "Sea", "Spring", "Stream", "Swamp", "Well", "Falls",
}

MAX_IMPORT_ROWS_BY_LAYER = {
    "nhdflowline_pr": 2500,
    "catchment_fabric_pr": 1500,
    "PRVI_Wetlands": 1500,
    "PuertoRico": 5,
}
SHAPEFILE_SPECS = {
    "BurnAddLine.shp": ("water", "nhdplus_burn_add_line", "line", "USGS NHDPlus"),
    "BurnAddWaterbody.shp": ("water", "nhdplus_burn_add_waterbody", "polygon", "USGS NHDPlus"),
    "BurnWaterbody.shp": ("water", "nhdplus_burn_waterbody", "polygon", "USGS NHDPlus"),
    "Sink.shp": ("water", "nhdplus_sink", "point", "USGS NHDPlus"),
    "Wall.shp": ("water", "nhdplus_wall", "line", "USGS NHDPlus"),
    "LandSea.shp": ("water", "nhdplus_land_sea", "polygon", "USGS NHDPlus"),
}
XLSX_SHEET_SPECS = {
    "HYDRO_FEATURES_FULL": ("water", "hydro_feature", "unknown", "PR Hydro Repository"),
    ...
    "EPA_NPDES_CATCHMENTS_PR": ("water", "npdes_catchment", "polygon", "EPA NPDES"),
    "EPA_ATTAINS_RECEIVING_WATERS_PR": ("water", "attains_receiving_water", "unknown", "EPA ATTAINS"),
    ...
    "SURFACE_LINEARWATER": ("water", "surface_linear_water", "line", "PR Hydro Repository"),
    ...
    "HL_WATERSHED_CONTEXT": ("water", "watershed_context", "unknown", "PR Hydro Repository"),
...
def _layer_spec(path: Path, layer: str) -> tuple[str, str, str, str] | None:
    ...
    if layer in NHDPLUS_LAYERS:
        return NHDPLUS_LAYERS[layer]
    if layer in WETLAND_LAYERS:
        return WETLAND_LAYERS[layer]
    if layer in GAZETTEER_LAYERS:
        return GAZETTEER_LAYERS[layer]
    return None


def _gazetteer_is_hydro(row: dict[str, Any]) -> bool:
    feature_class = clean(pick(row, ["feature_class", "FEATURE_CLASS", "class"]))
    description = clean(pick(row, ["description", "DESCRIPTION"])) or ""
    if feature_class in GAZETTEER_WATER_CLASSES:
        return True
    haystack = f"{feature_class or ''} {description}".lower()
    return any(token in haystack for token in ["water", "river", "reservoir", "lake", "stream", "canal", "spring", "well", "swamp", "bay", "harbor"])
...
def iter_gpkg(path: Path, layers: list[str] | None = None) -> Iterable[dict[str, Any]]:
    ...
    for layer in selected:
        ...
        for n, (idx, rec) in enumerate(gdf.iterrows()):
            ...
            if layer in GAZETTEER_LAYERS and not _gazetteer_is_hydro(row):
                continue
