...
def validate(path: Path, require_sha: bool) -> list[str]:
    ...
    with path.open('r', encoding='utf-8-sig', newline='') as f:
        ...
        for n, row in enumerate(reader, start=2):
            ...
            seen.add(cid)
            try:
                r = int(row['Row_Index']); c = int(row['Column_Index'])
                dark = int(row['Dark_Pixel_Count']); total = int(row['Total_Pixel_Count'])
                ratio = float(row['Land_Pixel_Ratio'])
            except Exception as exc:
                errors.append(f'invalid numeric field at line {n}: {exc}')
                continue
            rows.add(r); cols.add(c)
            if not 0 <= r <= 255: errors.append(f'Row_Index out of bounds at line {n}: {r}')
            if not 0 <= c <= 383: errors.append(f'Column_Index out of bounds at line {n}: {c}')
            if dark < 0 or total <= 0 or dark > total: errors.append(f'invalid pixel counts at line {n}')
            if not 0.0 <= ratio <= 1.0: errors.append(f'Land_Pixel_Ratio out of range at line {n}: {ratio}')
            if row.get('Classification', '') not in CLASSES: errors.append(f'unexpected Classification at line {n}')
    if count != 98304: errors.append(f'unexpected row count: {count}')
    if len(seen) != 98304: errors.append(f'unexpected unique Cell_ID count: {len(seen)}')
    if rows != set(range(256)): errors.append('Row_Index coverage is not complete 0..255')
    if cols != set(range(384)): errors.append('Column_Index coverage is not complete 0..383')
    if require_sha and file_sha(path) != SHA: errors.append('unexpected SHA-256')
    return errors
