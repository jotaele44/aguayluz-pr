...
This is an acquisition/evidence utility, not runtime drought logic. It downloads
source bytes without interpreting one drought class as another, records SHA-256
and retrieval metadata, and fails closed when a declared denominator is incomplete.

The tool deliberately separates three operations:

* NCEI GHCN-D station-universe discovery: filter the complete station catalog to
  Puerto Rico, then the complete inventory to PRCP series overlapping 2014-2016.
* USDM archive acquisition: freeze the three annual official GML archives and
  require exactly the 156 expected Tuesday issue dates when inspecting members.
* USGS acquisition plans: preserve exact OGC API request URLs/parameter semantics;
  observations are not certified until their downloaded bytes are present and hashed.

Large raw objects belong in a content-addressed evidence directory, not in Git.
The committed manifest contains query identity, byte count, SHA-256 and status.
"""
...
def parse_ghcnd_pr_station_ids(text: str) -> set[str]:
    """Parse fixed-width GHCN-D station catalog and return state=PR station IDs."""
    result: set[str] = set()
...
def parse_ghcnd_prcp_inventory(
    ...
    *,
    ...
    """Return PRCP stations whose published record overlaps the research window."""
...
def freeze_url(source_id: str, url: str, destination: Path) -> FrozenObject:
    ...
    return FrozenObject(source_id=source_id, **{k: v for k, v in asdict(frozen).items() if k != "source_id"})


def build_usgs_request_plan() -> list[dict[str, Any]]:
    """Return exact OGC request identities without claiming downloaded coverage.

    Parameter semantics are deliberately explicit. Daily/field data must be joined
    to metadata by identifiers; a parameter code's physical direction must be
    adjudicated before it can participate in a drought rule.
    """
...
def freeze_ncei(root: Path) -> tuple[list[FrozenObject], dict[str, Any]]:
    ...
    pr_ids = parse_ghcnd_pr_station_ids(stations_path.read_text(encoding="ascii", errors="replace"))
    ...
    # Preserve the exact denominator before large per-station acquisition.
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="aguayluz-freeze-"):
        if args.ncei:
            try:
                objects, ncei_result = freeze_ncei(root)
                frozen.extend(objects)
            except Exception as exc:  # terminal acquisition ledger; do not suppress other family
                failures.append({"family": "ncei", "error_type": type(exc).__name__, "error": str(exc)})
        if args.usdm:
            try:
                objects, usdm_result = freeze_usdm(root)
                frozen.extend(objects)
            except Exception as exc:  # terminal acquisition ledger; do not suppress other family
                failures.append({"family": "usdm", "error_type": type(exc).__name__, "error": str(exc)})
