...
def _adjacency(edges: Iterable[DirectedEdge]) -> tuple[dict[str, set[str]], dict[str, set[str]]]:
    ...
    for e in edges:
        if e.upstream_id == e.downstream_id:
            continue
        down[e.upstream_id].add(e.downstream_id)
        up[e.downstream_id].add(e.upstream_id)
    return down, up


def trace_downstream(
    start_id: str, edges: Iterable[DirectedEdge], *, max_hops: int = 1000
) -> list[str]:
    down, _ = _adjacency(edges)
    return _trace(start_id, down, max_hops)


def trace_upstream(
    start_id: str, edges: Iterable[DirectedEdge], *, max_hops: int = 1000
) -> list[str]:
    _, up = _adjacency(edges)
    return _trace(start_id, up, max_hops)
...
def impacted_assets(
    ...
    asset_to_segment: Mapping[str, str],
    *,
    direction: str = "downstream",
) -> list[str]:
    trace = set(
        trace_downstream(start_id, edges)
        if direction == "downstream"
        else trace_upstream(start_id, edges)
    ) | {start_id}
    return sorted(
        asset_id for asset_id, segment_id in asset_to_segment.items() if segment_id in trace
    )
...
def raster_layer_contract(spec: RasterSpec) -> dict:
    ...
    return {
        ...
        "nodata": spec.nodata,
        "band": spec.band,
        "logical_sha256": spec.logical_sha256,
    }
