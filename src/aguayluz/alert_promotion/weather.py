...
def weather_alert(
    ...
    # NWS hazards are area-based (no point epicenter), so link by municipality across
    # every named area, not by radius.
    linked, sectors = link_impact(
        None, None, areas, index, radius_km=MODULE_RADIUS_KM["WEATHER_HAZARD"]
    )
...
def weather_alerts(
    ...
    out: list[AlertEvent] = []
    for ev in events:
        alert = weather_alert(ev, geo, index)
