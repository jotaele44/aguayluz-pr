...
TaxonomicConfidence = Literal["specimen_verified", "expert_verified", "photo_supported", "reported", "unknown"]
...
def append_source(
    ...
    *,
    ...
    conn.execute(
        ...
        (source_id, title, source_type, source_uri, utc_now(), input_sha256, license_name, notes),
    )
    conn.commit()


def append_occurrence(conn: sqlite3.Connection, record: OccurrenceRecord) -> Literal["inserted", "duplicate"]:
    errors = validate_occurrence(record)
    if errors:
        raise ValueError({"error": "invalid_occurrence", "violations": errors})
    payload = json.dumps(record.canonical_payload(), sort_keys=True, separators=(",", ":"), ensure_ascii=False)
    try:
        conn.execute(
            "INSERT INTO occurrences(occurrence_id,fingerprint,schema_version,payload_json,source_id,appended_at) VALUES(?,?,?,?,?,?)",
            (record.occurrence_id, record.fingerprint(), record.schema_version, payload, record.source_id, utc_now()),
        )
        conn.commit()
        return "inserted"
...
def register_dataset(
    ...
    *,
    ...
    conn.execute(
        ...
        (dataset_id, title, version, sha256, status, utc_now(), json.dumps(metadata or {}, sort_keys=True)),
    )
    conn.commit()
...
def safe_occurrence_view(record: OccurrenceRecord, *, authorized_sensitive: bool = False) -> dict[str, Any]:
...
def analytics_unavailable(capability: str) -> dict[str, Any]:
    ...
    return {
        ...
        "available": False,
        "reason": "Phase 0 stores and validates evidence only; no calibrated ecological model is installed.",
    }
