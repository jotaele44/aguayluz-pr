...
def execute_source(
    ...
    *,
    ...
    execution = classify_execution(source)
    receipt["execution_class"] = execution
    if execution == "BLOCKED":
        receipt["state"] = str(source.get("adapter_state") or "BLOCKED_NO_ADAPTER")
        return [], receipt
    if execution == "REFERENCE_OR_CROSSCHECK":
    ...
    try:
        if adapter == "contentdm_api":
            ...
            receipt["state"] = (
                "PASS_DISCOVERY" if pagination["pagination_state"] == "PASS" else "UNRESOLVED_PAGINATION"
            )
        else:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="USACE Puerto Rico hydospatial harvester v3")
    ...
    headers = {"User-Agent": "aguayluz-pr-usace-harvester/3.0 (+public research corpus)"}
    with httpx.Client(timeout=httpx.Timeout(60, connect=20), headers=headers, follow_redirects=True) as client:
        ...
        if args.download_direct:
            for candidate in candidates:
                ...
                direct = clean.endswith((".pdf", ".doc", ".docx", ".xls", ".xlsx", ".zip", ".kmz", ".kml"))
                if not direct:
                    retrievals.append({
    ...
    required = [row for row in source_receipts if row.get("role") in PRIMARY_ROLES]
    blocking = [
        row for row in required
        if not str(row.get("state", "")).startswith("PASS_")
    ]
    snapshot = {
        ...
        "certification_state": (
            "OPEN_SOURCE_BLOCKERS" if blocking else "OPEN_MANIFESTATION_AND_IDENTITY_ADJUDICATION"
        ),
        "certified": False,
    }
